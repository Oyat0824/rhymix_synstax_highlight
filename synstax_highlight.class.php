...
class synstax_highlight extends EditorHandler
{
	...
	private function generateCopyButtonScript(): string
	{
		...
		$js .= 'if(btn.classList.contains("copy-message")) return;'.PHP_EOL;
		$js .= 'var existingTimer = btn.getAttribute("data-copy-timer");'.PHP_EOL;
		$js .= 'if(existingTimer) { clearTimeout(parseInt(existingTimer)); }'.PHP_EOL;
		$js .= 'var resetTimer = btn.getAttribute("data-reset-timer");'.PHP_EOL;
		$js .= 'if(resetTimer) { clearTimeout(parseInt(resetTimer)); }'.PHP_EOL;
		$js .= 'var clickCount = parseInt(btn.getAttribute("data-copy-count") || "0");'.PHP_EOL;
		$js .= 'clickCount++;'.PHP_EOL;
		$js .= 'btn.setAttribute("data-copy-count", clickCount);'.PHP_EOL;
		$js .= 'var resetTimerId = setTimeout(function() { btn.setAttribute("data-copy-count", "0"); btn.removeAttribute("data-reset-timer"); }, 10000);'.PHP_EOL;
		$js .= 'btn.setAttribute("data-reset-timer", resetTimerId);'.PHP_EOL;
		$js .= 'var messages = ["복사 완료!", "또 복사?", "계속 복사하네?", "진짜 많이 복사하시네...", "복사 마니아시군요!", "복사 중독자 발견!", "복사왕 등장!", "복사 신이시네요!", "복사의 신!!!!!!!!!!"];'.PHP_EOL;
		$js .= 'var messageIndex = Math.min(clickCount - 1, messages.length - 1);'.PHP_EOL;
		$js .= 'var message = messages[messageIndex];'.PHP_EOL;
		$js .= 'var shakeClass = "";'.PHP_EOL;
		$js .= 'if(clickCount >= 3 && clickCount < 7) { shakeClass = "copy-shake"; }'.PHP_EOL;
		$js .= 'else if(clickCount >= 7) { shakeClass = "copy-shake-intense"; }'.PHP_EOL;
		$js .= 'var copySuccess = function() {'.PHP_EOL;
		$js .= 'var originalText = btn.getAttribute("data-original-text") || "Copy";'.PHP_EOL;
		$js .= 'if(!btn.getAttribute("data-original-text")) { btn.setAttribute("data-original-text", originalText); }'.PHP_EOL;
		$js .= 'btn.textContent = message;'.PHP_EOL;
		$js .= 'btn.classList.add("copy-message");'.PHP_EOL;
		$js .= 'if(shakeClass) { btn.classList.add(shakeClass); }'.PHP_EOL;
		$js .= 'var timer = setTimeout(function() { btn.textContent = originalText; btn.classList.remove("copy-message"); if(shakeClass) { btn.classList.remove(shakeClass); } btn.removeAttribute("data-copy-timer"); }, 2500);'.PHP_EOL;
		$js .= 'btn.setAttribute("data-copy-timer", timer);'.PHP_EOL;
		$js .= '};'.PHP_EOL;
		$js .= 'if(navigator.clipboard && navigator.clipboard.writeText) {'.PHP_EOL;
		$js .= 'navigator.clipboard.writeText(text).then(copySuccess).catch(function(err) { });'.PHP_EOL;
		$js .= '} else {'.PHP_EOL;
		...
		$js .= 'try { document.execCommand("copy"); copySuccess(); } catch(err) { }'.PHP_EOL;
